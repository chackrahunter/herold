#!/bin/bash
# Herold - baut die APK ohne Gradle, direkt mit aapt2/d8/apksigner.
#   ./build.sh            nur bauen
...
echo "[5/6] APK packen"
cp build/base.apk build/herold-unsigned.apk
(cd build && zip -q herold-unsigned.apk classes.dex)
"$BT/zipalign" -f 4 build/herold-unsigned.apk build/herold-aligned.apk

echo "[6/6] Signieren"
KS=herold.keystore   # ausserhalb von build/, damit die Signatur stabil bleibt
if [ ! -f "$KS" ]; then          # nur einmal erzeugen - sonst aendert sich die Signatur
  "$JAVA_HOME/bin/keytool" -genkeypair -keystore "$KS" -storepass herold -keypass herold \
    -alias herold -keyalg RSA -keysize 2048 -validity 10000 \
    -dname "CN=Herold" >/dev/null 2>&1
fi
"$BT/apksigner" sign --ks "$KS" --ks-pass pass:herold --key-pass pass:herold \
  --out build/herold.apk build/herold-aligned.apk

echo "FERTIG: $(pwd)/build/herold.apk ($(du -h build/herold.apk | cut -f1))"
...
if [ "$1" = "--install" ] || [ "$1" = "--selftest" ]; then
  ...
  adb -t "$W" install -r -g build/herold.apk
fi

if [ "$1" = "--selftest" ]; then
  W=$(watch_id)
  echo "--- Selbsttest ---"
  adb -t "$W" shell am start -n de.doncalvin.herold/.MainActivity >/dev/null
  sleep 3
  FG=$(adb -t "$W" shell dumpsys activity activities | tr -d '\r' | grep -o 'de.doncalvin.herold/[^ }]*' | head -1)
  ...
  echo "--- Log ---"
  adb -t "$W" logcat -d -s Herold:I | tail -15
  case "$FG" in
    de.doncalvin.herold*) echo "SELBSTTEST OK" ;;
